...
export function sessionCookie(value, maxAge, secure = true) {
  const parts = [`cms_session=${value}`, 'Path=/', 'HttpOnly', 'SameSite=Strict', `Max-Age=${maxAge}`];
  if (secure) parts.push('Secure'); // browsers drop Secure cookies over http://localhost; off only in local dev
  return parts.join('; ');
}
