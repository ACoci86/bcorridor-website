...
(function () {
  ...
  // Chrome race fix: with `autoplay` in the markup, playback can begin —
  // and the one-off 'playing' event can fire — before this deferred script
  // runs (it queues behind the HTML parse and leaflet.js). If that already
  // happened, the listener above never fires and the video would stay at
  // opacity 0 forever. Reveal on the first composited frame instead, and
  // fall back to a state check on browsers without rVFC.
...
(function () {
  ...
  const FALLBACKS = [
    {
      ...
      image:
        "https://images.unsplash.com/photo-1568526381923-caf3fd520382?w=400&q=80",
  ...
  function factTypeIconSrc(value) {
    ...
    if (type.includes("plant") || type.includes("flora"))
      return "assets/plant.svg";
    if (type.includes("bird") || type.includes("aves"))
      return "assets/bird.svg";
    if (
      type.includes("insect") ||
      type.includes("pollinator") ||
      type.includes("bee") ||
  ...
  function factIconClass(item) {
    ...
    if (label === "type" && value.includes("bird"))
      return " fact-row-icon--bird";
    return "";
  }
  ...
  function renderQuickFacts(facts) {
    ...
    factList.innerHTML = usableFacts
      .map((item) => {
        ...
        return (
          ...
          '">' +
          icon +
          '<span class="fact-term">' +
          escapeHtml(item.label) +
          "</span>" +
          '<span class="fact-desc">' +
          escapeHtml(value) +
          "</span>" +
          "</li>"
        );
    ...
    try {
      ...
      for (let attempt = 0; attempt < MAX_TRIES; attempt++) {
        ...
        const usable = (obsData.results || []).filter((o) => {
          if (!o.taxon || !o.taxon.id || !o.photos || o.photos.length === 0)
            return false;
          if (
            !["Plantae", "Insecta", "Aves"].includes(o.taxon.iconic_taxon_name)
          )
            return false;
        ...
        for (let i = 0; i < Math.min(5, shuffled.length); i++) {
          ...
          try {
            const taxonData = await fetchWithTimeout(
              "https://api.inaturalist.org/v1/taxa/" + obs.taxon.id,
              6000,
            );
            ...
            const wikiUrl =
              full.wikipedia_url ||
              "https://www.inaturalist.org/taxa/" + full.id;
...
(function () {
  const quotes = [
    {
      text: "We may rebuild our cathedrals and our castles, but we cannot rebuild a meadow once it is lost.",
      author: "John Feehan, Irish ecologist & author",
    },
    {
      text: "The bog is more than a landscape: it is the slowest archive we have, and what we lose from it we lose forever.",
      author: "Tim Robinson, Irish writer & cartographer",
    },
    {
      text: "Ireland's hedgerows are our linear forests. They thread the country together, and if we let them go, the country comes apart.",
      author: "Éanna Ní Lamhna, Irish biologist & broadcaster",
    },
    {
      text: "Pollinators ask very little of us: a strip of clover, a corner left unmown, a single native tree. That is all it takes to begin.",
      author: "All-Ireland Pollinator Plan",
    },
    {
      text: "We have inherited a wild Ireland of great beauty. We must hand it on richer than we found it.",
      author: "Michael Viney, Irish Times nature columnist",
    },
  ];
...
(function () {
  ...
  const corridorArea = [
    [52.25554, -7.0974779],
    [52.2553824, -7.1025419],
    [52.2541214, -7.1034003],
    [52.2520196, -7.1028853],
    [52.2477108, -7.0965338],
    [52.2466072, -7.0876932],
    [52.245924, -7.0833159],
    [52.2452409, -7.0803976],
    [52.2442949, -7.0765353],
    [52.2426657, -7.0744753],
    [52.2402481, -7.0713854],
    [52.2368842, -7.0659781],
    [52.2348342, -7.0638323],
    [52.2321007, -7.0652056],
    [52.2287363, -7.0648623],
    [52.228158, -7.05688],
    [52.2307865, -7.0549917],
    [52.2337303, -7.0577383],
    [52.2364637, -7.0562792],
    [52.2408262, -7.058425],
    [52.2430336, -7.0597983],
    [52.2461342, -7.0603133],
    [52.2473429, -7.0626307],
    [52.2482363, -7.0657206],
    [52.2498127, -7.0666647],
    [52.251862, -7.0666647],
    [52.2534909, -7.0664072],
    [52.2552773, -7.0672655],
    [52.2560654, -7.0688963],
    [52.2558552, -7.0721579],
    [52.2544892, -7.0771361],
    [52.2527553, -7.0828867],
    [52.2516518, -7.0860625],
    [52.2521247, -7.0924998],
    [52.2538587, -7.0955897],
    [52.25554, -7.0974779],
  ];
  ...
  const stadiaQuery = STADIA_API_KEY
    ? "?api_key=" + encodeURIComponent(STADIA_API_KEY)
    : "";
  const stadiaAttribution =
    '&copy; <a href="https://www.stadiamaps.com/" target="_blank" rel="noopener">Stadia Maps</a> &copy; <a href="https://stamen.com/" target="_blank" rel="noopener">Stamen Design</a> &copy; <a href="https://openmaptiles.org/" target="_blank" rel="noopener">OpenMapTiles</a> &copy; <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener">OpenStreetMap</a> contributors';
  ...
  const stopButtons = Array.from(
    document.querySelectorAll(".stop-button[data-stop]"),
  );
  ...
  stops.forEach((stop) => {
    const marker = L.marker(stop.coords, {
      icon: numberedIcon(stop.num),
      title: stop.num + ". " + stop.fullName,
    }).addTo(map);
  ...
  function updateStopDetail(stop) {
    ...
    if (detailEyebrow)
      detailEyebrow.textContent = "Stop " + String(stop.num).padStart(2, "0");
  ...
  function closeDetail() {
    ...
    if (
      mqSheet.matches &&
      detailEl &&
      mapPanel.classList.contains("is-detail-open")
    ) {
  ...
  function setActiveStop(stopNumber, options = {}) {
    ...
    stopButtons.forEach((b) => {
      ...
      if (isActive && options.scrollStrip !== false)
        b.scrollIntoView({
          behavior: "smooth",
          block: "nearest",
          inline: "center",
    ...
    markersByStop.forEach((marker, num) => {
      if (marker._icon)
        marker._icon.classList.toggle("is-active", num === String(stopNumber));
    });
  ...
  function moveStop(delta) {
    const currentIndex = stops.findIndex(
      (s) => String(s.num) === String(activeStopNumber),
    );
  ...
  resetControl.onAdd = function () {
    ...
    btn.innerHTML =
      '<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false"><path d="M3 12a9 9 0 1 0 3-6.7"/><polyline points="3 4 3 9 8 9"/></svg>';
    ...
    try {
      ...
      if (data && data.code === "Ok" && data.routes && data.routes[0]) {
        ...
        routeLayer = L.polyline(
          path,
          Object.assign({}, routeStyle, { color: lineColor }),
        ).addTo(map);
      }
    } catch (err) {
      console.warn(
        "[B Corridor map] OSRM routing unavailable, keeping straight-line route:",
        err,
      );
    } finally {
  ...
  (() => {
    ...
    const rotate = (dir) => {
      const current = polaroids.map((p) =>
        SLOTS.findIndex((s) => p.classList.contains(s)),
      );
    ...
    group
      .querySelector(".polaroid-nav--prev")
      ?.addEventListener("click", () => rotate(-1));
    group
      .querySelector(".polaroid-nav--next")
      ?.addEventListener("click", () => rotate(1));
  })();
})();
...
(function newsFilterPagination() {
  ...
  const triggers = Array.from(
    document.querySelectorAll(
      ".blog-filter__item[data-filter], .cat-link[data-filter]",
    ),
  );
  ...
  cards.forEach((c) => {
    const k = c.getAttribute("data-category");
  ...
  if (pager) grid.insertBefore(empty, pager);
  ...
  const filtered = () =>
    cards.filter(
      (c) => filter === "all" || c.getAttribute("data-category") === filter,
    );
    ...
    wanted.forEach((p, i) => {
      ...
      if (i < wanted.length - 1) {
        ...
        if (gap === 2) items.push({ type: "page", n: p + 1 });
          ...
          items.push({
            type: "gap",
            jump: Math.round((p + wanted[i + 1]) / 2),
          });
      }
    });
    return items;
  ...
  function renderPager(total) {
    ...
    pager.hidden = false;
    if (page > 1)
      btn("blog-pagination__nav", "&lsaquo; Prev", "Previous page", () =>
        goTo(page - 1),
      );
    pageItems(page, total).forEach((it) => {
      if (it.type === "page")
        ...
        btn(
          ...
          "&hellip;",
          "Jump to page " + it.jump,
          () => goTo(it.jump),
        );
    });
    if (page < total)
      btn("blog-pagination__nav", "Next &rsaquo;", "Next page", () =>
        goTo(page + 1),
      );
  }
  ...
  function render(scroll) {
    ...
    list.slice(start, start + PAGE_SIZE).forEach((c) => {
      c.style.display = "";
    });
    ...
    if (scroll)
      (document.querySelector(".blog-filter") || grid).scrollIntoView({
        behavior: "smooth",
        block: "start",
      });
  }
